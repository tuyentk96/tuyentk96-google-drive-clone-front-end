...
import { addStar, changePublic, checkPermissionShare, deleteFolderOrFile, disableStar, download, getFolderOrFile, getListStar, getStorage, renameFolderOrFile, shareDataforAnotherUser } from "../../../service/uploadService"
...
const MyDrive = () => {
    const [listMyDrive, setListMyDrive] = useState([])
    ...
    const fetchStorage = async () => {
        const response1 = await getStorage({ folderId: storageId, userId });
    ...
    const handleOnClickDownload = async (e, folderId) => {
        e.preventDefault()

        const folderOrFile = await getFolderOrFile(folderId)

        if (folderOrFile.statusCode === 200) {
            const downloadFile = await download(folderId);

            if (folderOrFile.metadata.type === 'file') {

                fileDownload(downloadFile.data, folderOrFile.metadata.nameFile)
            }

            if (folderOrFile.metadata.type === 'folder') {

                fileDownload(downloadFile.data, `${folderOrFile.metadata.nameFile}.zip`)
            }
        }
    };
            ...
            <div className="home-container text-[#1F1F1F] pl-4">
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div>
                            {listMyDrive && listMyDrive.map((folder, index) => {
                                return (
                                    <>
                                        ...
                                        <div className="hover:bg-[#F0F4F8] group flex h-[48px] border-b-[1px] items-center text-[14px]" onDoubleClick={folder.nameFolder ? () => handleClickFolder(folder._id) : () => handleClickFile(folder._id)} key={index}>
                                            ...
                                            <div className="hidden group-hover:flex gap-2 w-[15%]">
                                                ...
                                                <div className="hover:cursor-pointer hover:bg-[#e9eef6] p-[6px] rounded-full"
                                                    onClick={(e) => handleOnClickDownload(e, folder._id)}
                                                >
                                                ...
                                                <div
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        const updatedList = listMyDrive.map((item) =>
                                                            item._id === folder._id
                                                                ? { ...item, isStar: !item.isStar }
                                                                : item
                                                        );
                                                        setListMyDrive(updatedList);
                                                    }}
                                                >
