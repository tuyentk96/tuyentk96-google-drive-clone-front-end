...
import { addStar, checkPermissionShare, disableStar, getFolderShareWithMe, getListStar, getStorage, shareDataforAnotherUser } from "../../../service/uploadService"
...
const FolderShareWithMe = () => {
    ...
    const storageId = useSelector(state => state.folder.storageId)
    ...
    const fetchStorage = async () => {
        if (currentFolderId && userId) {
            const response1 = await getStorage({ folderId: folderId, userId });
    ...
    const fetchFolderShareWithMe = async (id) => {
        const response = await getFolderShareWithMe({ folderId: id, userId });
    ...
    useEffect(() => {
        fetchFolderShareWithMe(currentFolderId);
    }, [currentFolderId])
        ...
        <>
            <div className="home-container text-[#1F1F1F] pl-4">
                <div className="home-title  pt-[14px] pb-[6px] text-[24px] flex gap-3 items-center">
                    <div className="hover:bg-[#f0f1f1] px-2 rounded hover:cursor-pointer" onClick={handleClickShareWithMe}>
                        Share With Me
                    </div>
                    <div>
...
export default FolderShareWithMe