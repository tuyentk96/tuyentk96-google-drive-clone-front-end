...
import { getFolderPublic } from "../../service/accessService"


const Public = () => {
    const [listFolderPublic, setListFolderPublic] = useState([])
    ...
    useEffect(() => {
        const fetchFolderPublic = async () => {
            const response = await getFolderPublic(folderId)
            console.log(response);

            if (response.statusCode === 200) {
                setListFolderPublic(response.metadata.data)
                setFolderParrent(response.metadata.folder)
            }
        }
        fetchFolderPublic();
    ...
    return (
        <>
            <div className="home-container text-[#1F1F1F] pl-4">
                ...
                <div className="home-title flex pt-[14px] pb-[6px] text-[24px] gap-3">
                    <div>
                        {folderParrent && folderParrent?.nameFolder ? `Folder: ${folderParrent?.nameFolder}` : `File: ${folderParrent?.nameFile}`}
                    </div>
                ...
                <div>
                    ...
                    <div>
                        ...
                        {listFolderPublic && <div>
                            {listFolderPublic.map((folder, index) => {
                                return (
                                    <>
